...
def findEncodingImg(images):
    ...
    return encodeList
def  Mark_Face_Recognition(name):#save data name attendance into csv file
    with open('Face_Recognition001.csv','r+')as f:
...
while True:
    ...
    for encodeFace,faceLoc in zip(encodeCurFrame, faceCurFrame):
        ...
        if(matches[matcheIndexes]):
            ...
            Mark_Face_Recognition(name)
            #check_name_state(name)
    
    cv2.putText(img, '' , (10,18), cv2.FONT_HERSHEY_COMPLEX, 0.8, (0,0,255),2)
